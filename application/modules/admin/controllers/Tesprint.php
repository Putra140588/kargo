...
class Tesprint extends CI_Controller{
	...
	function index($id='',$idbarcode=''){
		...
		if (!empty($id)){		
			...
			if (count($sql) > 0){
				...
				//$customPaper = array(0,0,269.291338583,170.078740157);//95 mm, 60 mm or 10 cm, 6cm convert to pont(computer)
	...
	function rekap_report($tab,$print=false){
		...
		$body='';
		if ($tab == 'tab1'){								
			$title = 'Rekapitulasi Pendapatan Barcode Label';
			...
			if (count($data['rekap']) > 0){
				...
				foreach ($byshift as $rx){
					$xc[] = ISOCODE.' '.number_format($rx->total_price).' ('.$rx->id_shift.'-'.$rx->total_qty.')';
				}
			...
			if (count($report) > 0){
				...
				if (count($det) > 0){				
					foreach ($det as $v){					
						$xt[] = $v->total_qty.' ('.$v->id_shift.')';
						$xc[] = ISOCODE.' '.number_format($v->total_price).' ('.$v->id_shift.')';
						$xx[] = ISOCODE.' '.number_format($v->total_amount_tax).' ('.$v->id_shift.')';
						$xz[] = ISOCODE.' '.number_format($v->total_price_tax).' ('.$v->id_shift.')';
					}
				}
			...
			$title = 'Laporan Penerimaan Barcode Label General';
			$filename = $title.' '.short_date($from).'-'.short_date($to);			
			$data['title'] = $title;
			$data['title2'] = 'Periode '.short_date($from).' - '.short_date($to);
			$data['order'] = $this->m_admin->report_general($from,$to);
			$sum = $this->m_admin->report_general($from,$to,true);
			if (count($sum) > 0){
				foreach ($sum as $row)
					foreach ($row as $key=>$val){
					$data[$key] = $val;
