		...
		success	: function (param){		
			 ...
			 if (json.error == 0){
				 if (json.edit != null){					 
					 ...
					 //for use in stock management
					 $("#"+div).html(json.label);
					 $('#productatt').html(json.prod_attr);
					 $("#whlc").html(json.whlc);
				 }else if (json.zone != null){
					 //for use in carrier zone					
					 $('#'+div).html(json.tbl1);
					 $("#tbl2").html(json.tbl2);
				 }else if (json.parent != null){	
					 //select for parent categories
					 if (div == 'parent'){
						 var x = $(".parent").val($(".parent option:first").val());							
					 }					
					 $("#"+div).html(json.element);
				 }else if (json.shippzone != null){	
					 $("#districtcover").html(json.content);
					 $("#"+div).html(json.element);
					 
				 }else if (json.districtarea != null){	
					 $("#"+json.contentid).html(json.content);				
					 
				 }else{
					 if (div == 'parent'){
						 //for get default selectitem categori
						 $('.parent').get(1).selectedIndex = 0;//level1
						 $('.parent').get(2).selectedIndex = 0;//level2
					 }else if (div == 'finalprice' || div == 'friendlyurl'){
						 $("#"+div).val(json.element);
					 }					 
					//show element general html				
					 $("#"+div).html(json.element);
					 load_input_image();
				 }					 				 		 
